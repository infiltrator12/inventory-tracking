...
$(document).ready(function(){
    ...
    function searchData(searchTerm){
        $.ajax({
            url: '/inventory_Tracking/php/conditionsdatabase.php?function=search&q='+searchTerm,
            method: 'POST',
            datatype: 'json',
        ...
        if(device.length > 0){
            for(var i = 0; i< device.length; i++){
                ...
                tableBody.innerHTML +=`
                    ...
                    <td> ${devices.employeesurname} </td>
                    <td> ${devices.condition} </td>
                    <td>
...
function deleteData(element){
    ...
    if(confirmation){
        ...
        xhr.open("POST", "/inventory_Tracking/php/conditionsdatabase.php?function=delete", true);
...
function editData(element){
    ...
    var condition = row.cells[4].innerHTML;
    ...
    document.getElementById("editmodel").value = employeenumber;
    document.getElementById("editstoragecapacity").value = employeename;
    document.getElementById("editconditions").value = condition;
    //display the form
    openForm();
}
function openForm(){
    var formContainer = doument.getElementById("editFormContainer");
...
function update(){
    ...
    var xhr = new XMLHttpRequest();
    xhr.open("POST", "/inventory_Tracking/php/conditionsdatabase.php?function=update", true);
    xhr.setRequestHeader("content-type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function(){
        if(xhr.readyState === 4 && xhr.status === 200){
