...
$(document).ready(function(){
    ...
    function searchData(searchTerm){
        $.ajax({
            url: '/inventory_Tracking/php/historydatabase.php?function=search&q='+searchTerm,
            method: 'POST',
            datatype: 'json',
        ...
        if(device.length > 0){
            for(var i = 0; i< device.length; i++){
                ...
                tableBody.innerHTML +=`
                    ...
                    <td> ${devices.startdate} </td>
                    <td> ${devices.enddate} </td>
                    <td>
...
function deleteData(element){
    ...
    if(confirmation){
        ...
        xhr.open("POST", "/inventory_Tracking/php/historydatabase.php?function=delete", true);
...
//     var row = element.parentNode.parentNode;
//     var serialnumber = row.getAttribute("DEVICE_SERIALNUMBER");
//     //extract data from the row
//     var serialnumber = row.cells[0].innerHTML;
//     var assettag = row.cells[1].innerHTML;
//     var employeenumber = row.cells[2].innerHTML;
//     var employeename = row.cells[3].innerHTML;
//     var startdate = row.cells[4].innerHTML;
//     var enddate = row.cells[5].innerHTML;
...
//     formData.append("editserialnumber",serialnumber);

//     // Make an AJAX request to update the data
//     var xhr = new XMLHttpRequest();
//     xhr.open("POST", "/inventory_Tracking/php/historydatabase.php?function=update", true); 
//     xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
//     xhr.onreadystatechange = function() {
//         if (xhr.readyState === 4 && xhr.status === 200) {
